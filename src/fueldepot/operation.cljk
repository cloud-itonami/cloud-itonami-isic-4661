(ns fueldepot.operation
  "FuelDepotOperationActor -- one depot-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (FuelDepotAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Fuel Depot
  ...
  each operation is auditable and checkpointed. A depot's lifecycle is
  advanced by MANY ops (shipment-record logging / delivery-operation
  scheduling / supplier-order coordination), each independent.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human depot supervisor/procurement approver. The
...
(defn build
  "Compiles a FuelDepotOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `fueldepot.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FuelDepotAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Fuel Depot Operations Governor -- independent censor.
      (g/add-node :govern
